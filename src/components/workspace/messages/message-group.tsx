...
function ToolCall({
  ...
  if (name === "web_search") {
    ...
    if (typeof args.query === "string") {
      label = `Search on the web for "${args.query}"`;
    }
    return (
    ...
    let label: React.ReactNode = "Search for related images";
    if (typeof args.query === "string") {
      label = `Search for related images for "${args.query}"`;
    }
    const results = (
      result as {
        results: {
    ...
    return (
      <ChainOfThoughtStep
        key={id}
        label={description ?? `Use "${name}" tool`}
        icon={WrenchIcon}
      ></ChainOfThoughtStep>
    );
  }
